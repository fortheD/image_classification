...
class ResNetV1(object):
    ...
    def identity_block(self, input_tensor, kernel_size, filters, stage, block, data_format,
        ...
        x = Conv2D(filter1, (1, 1), name=conv_name_base+'2a', use_bias=use_bias, data_format=data_format)(input_tensor, training=training)
        ...
        x = Conv2D(filter2, (kernel_size, kernel_size), padding='same', name=conv_name_base+'2b', use_bias=use_bias, data_format=data_format)(x, training=training)
        x = BatchNormalization(axis=(-1, 1)[self.data_format=="channels_first"], name=bn_name_base+'2b')(x, training=training)
        x = Activation('relu')(x, training=training)

        x = Conv2D(filter3, (1, 1), name=conv_name_base+'2c', use_bias=use_bias, data_format=data_format)(x, training=training)
        x = BatchNormalization(axis=(-1, 1)[self.data_format=="channels_first"], name=bn_name_base+'2c')(x, training=training)

        x = Add()([x, input_tensor], training=training)
        x = Activation('relu', name='res'+str(stage)+block+'_out')(x, training=training)
        return x
    ...
    def conv_block(self, input_tensor, kernel_size, filters, stage, block, data_format,
        ...
        x = Conv2D(filter1, (1, 1), strides=strides, name=conv_name_base+'2a', use_bias=use_bias, data_format=data_format)(input_tensor, training=training)
        x = BatchNormalization(axis=(-1, 1)[self.data_format=="channels_first"], name=bn_name_base+'2a')(x, training=training)
        x = Activation('relu')(x, training=training)
        ...
        shortcut = Conv2D(filter3, (1, 1), strides=strides, name=conv_name_base+'1', use_bias=use_bias, data_format=data_format)(input_tensor, training=training)
        shortcut = BatchNormalization(axis=(-1, 1)[self.data_format=="channels_first"], name=bn_name_base+'1')(shortcut, training=training)

        x = Add()([x, shortcut], training=training)
        x = Activation('relu', name='res'+str(stage)+block+'_out')(x, training=training)
        return x
    ...
    def __call__(self, input_tensor, training):
        ...
        """
        # Stage 1
        x = ZeroPadding2D((3, 3), data_format=self.data_format)(input_tensor, training=training)
        x = Conv2D(64, (7, 7), strides=(2, 2), name='conv1', use_bias=True, data_format=self.data_format)(x, training=training)
        x = BatchNormalization(axis=(-1, 1)[self.data_format=="channels_first"], name='bn_conv1')(x, training=training)
        x = Activation('relu')(x, training=training)
        C1 = x = MaxPooling2D((3, 3), strides=(2, 2), padding="same", data_format=self.data_format)(x, training=training)
        # Stage 2
